...
public class PlayerUI : PopupBase
{
    ...
    {
        LeanTween.value(stressNeedle.gameObject, stressNeedle.localEulerAngles.z, GameManager.Inst.CurStress * 180f, 1.5f).setEase(LeanTweenType.easeOutCirc).setOnUpdate((float value) => { NeedleMove(value); });
    }
    [ContextMenu("Stress Renewal - Instant")]
    public void StressRenewal_Instant()
    {
        NeedleMove(GameManager.Inst.CurStress * 180f);
    }
